	     ...
	     )
{
    ...
    *listenSocket = socket(domain, SOCK_STREAM, 0);
    if(*listenSocket == -1)
	{
            ...
            LOGE("Warning: Cannot create server listen %s socket\nWarning is %d %s\n",
                   domain == AF_INET6 ? "IPv6" :
                   (domain == AF_INET) ? "IPv4" : "?", errno, strerror(errno));
		LOGE("Ignore the IPv6 warning if the platform doesn't support IPv6\n");
	    return -1;
	}
    ...
    if (res != 0) {
	LOGD("setsockopt error. Error is %d %s\n", errno, strerror(errno));
	return -1;
    }
	...
	*addr_len = sizeof(struct sockaddr_in6);
        ...
        res = setsockopt(*listenSocket, IPPROTO_IPV6, IPV6_V6ONLY, &opt,
			 sizeof(opt));
        if (res != 0) {
	    LOGD("setsockopt IPV6_V6ONLY error. Error is %d %s\n",
		   errno, strerror(errno));
	    return -1;
        }
	res= bind(*listenSocket,(struct sockaddr*) &MyAddress6, *addr_len);
      break;
      default:
        LOGD("Address family %d not supported\n", domain);
        return -1;
    }
    if(res != 0)
	{
    ...
    if(res != 0)
	{
	    close(*listenSocket);
	    *listenSocket = -1;
            LOGD("Listen error.  Error is %d %s\n", errno, strerror(errno));
	    return -1;
	}
    return 0;
}
	       ...
	       )
{
    ...
    for(;;)
	{
	    ...
	    switch(Command)
		{
		  ...
		  default:
		    LOGD("Unrecognized platform interface command %08x\n", Command);
		    WriteUINT32(s, 1);
		    return true;
		}
	    WriteUINT32(s,0);
		   ...
		   )
{
    ...
    if (nSock == 0) {
    	LOGD("Create platform service socket fail...\n");
		LOGD("Create platform service socket fail\n");
	return -1;
    }
    ...
    do {
    	LOGD("Platform server listening on port");
        LOGD("Platform server listening on port %d\n", PortNumber);
	...
	if (res == -1) {
	    LOGD("Platform server select error.  Error is %d %s\n",
		   errno, strerror(errno));
	    return -1;
	}
	...
	for (i = 0; i < nSock; i++) {
	    ...
	    if(serverSocket < 0)
		{
		    LOGD("Platform server IPv%d Accept error.  Error is %d %s\n",
			   ipver, errno, strerror(errno));
		    return -1;
		};
	    LOGD("Platform IPv%d client accepted\n", ipver);
		      ...
		      )
{
    ...
    if (nSock == 0) {
        LOGD("Create TPM command service socket fail\n");
        return -1;
    }
    maxListenSocket = listenSocket[0];
    if (nSock == 2 && listenSocket[1] > maxListenSocket) {
    ...
    do
	{
    	LOGD("TPM command server listening on port");
	    LOGD("TPM command server listening on port %d\n", *PortNumber);
	    ...
	    if (res == -1) {
	        LOGD("TPM command server select error.  Error is %d %s\n", errno,
		       strerror(errno));
	        return -1;
	    }
		...
		if(serverSocket < 0)
		    {
			LOGD("TPM server IPv%d Accept error.  Error is %d %s\n",
			       ipver, errno, strerror(errno));
			return -1;
		    };
	        LOGD("Command IPv%d client accepted\n", ipver);
	       ...
	       )
{
    ...
    if(!running)
	{
	    ...
	    if(ret != 0){
	    	LOGD("ACT creation failed\n");
			LOGD("ACT thread Creation failed\n");
		}
	    else {
			running = TRUE;
			LOGD("ACT Thread Running\n");
		}
	}
    return ret;
}
	       ...
	       )
{
    ...
    if (res != 0)
	{
	    LOGD("PlatformSignalService failed\n");
	    return res;
	}
    ...
    if (res != 0)
	{
	    LOGD("RegularCommandService failed\n");
	    return res;
	}
    return 0;
}
	     ...
	     )
{
    ...
    *BytesReceived = length;
    if(length>MaxLen)
	{
	    LOGD("Buffer too big.  Client says %d\n", length);
	    return false;
	}
    ...
    for(;;)
	{
	    ...
	    switch(Command)
		{
		  ...
		  case TPM_REMOTE_HANDSHAKE:
		    ...
		    if( clientVersion == 0 )
			{
			    LOGD("Unsupported client version (0).\n");
			    return true;
			}
		  ...
		  case TPM_STOP:
		    ...
		    return false;
		  default:
		    LOGD("Unrecognized TPM interface command %08x\n", Command);
		    return true;
		}
	    ok = WriteUINT32(s,0);
	    if(!ok)
