...
void printC_String(char *name, unsigned char *arr, size_t len) {
    printf("uint8_t %s[%zu] = {", name,len);
    for (int i = 0; i < len; i++){
        printf("0x%02x", arr[i]);
        if(i != (len-1)){
            printf(", ");
		    ...
		    )
{
    ...
    printC_String("CurrentPolicyDigest",session->u2.policyDigest.b.buffer,32);
    ...
    printC_String("name",name->b.buffer,32);
