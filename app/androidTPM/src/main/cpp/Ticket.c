		      ...
		      )
{
    ...
    ticket->digest.t.size = CryptHmacStart2B(&hmacState, CONTEXT_INTEGRITY_HASH_ALG,
					     &proof->b);
    //  TPM_ST_VERIFIED
    printf("Updating ticket with tag: %d\n", ticket->tag);
    CryptDigestUpdateInt(&hmacState, sizeof(TPM_ST), ticket->tag);
    //  digest
    printf("Updating ticket with digest: ");
    for(int i = 0; i < digest->b.size; i++){
    ...
    printf("Updating ticket with keyname: ");
    for(int i = 0; i < keyName->b.size; i++){
        printf("%02x ", keyName->b.buffer[i]);
    }
    printf("\n");
    // done
