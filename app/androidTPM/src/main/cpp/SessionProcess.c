	      ...
	      )
{
    ...
    printf("[*] Calculating CpHash for command: %04x\n", command->code);
    ...
    if(cpHash->t.size == 0)
	{
        ...
        printf("[+] Hashing and adding CC: %04x\n", command->code);
	    //  Add authNames for each of the handles.
	    for(i = 0; i < command->handleNum; i++) {
            printf("[+] Hashing and adding authName: ");
            TPM2B t = EntityGetName(command->handles[i],&name)->b;
            for(int r = 0; r < t.size; r++){
                printf("%02x ",t.buffer[r]);
            }
            printf("\n");
        ...
        printf("[+] Adding parameter buffer (size %d): ", command->parameterSize);
        for(int r = 0; r < command->parameterSize; r++) printf ("%02x ", command->parameterBuffer[r]);
        printf("\n");
	    //  Complete the hash.
	    CryptHashEnd2B(&hashState, &cpHash->b);
        printf("Final result: ");
        for(int r = 0; r < cpHash->b.size; r++) printf("%02x ", cpHash->b.buffer[r]);
        printf("\n");
	}
    return cpHash;
}
		    ...
		    )
{
    ...
    for(sessionIndex = 0; command->authSize > 0; sessionIndex++)
	{
        ...
        printf("Checking in session process\n");
    ...
    if(command->tag == TPM_ST_SESSIONS)
	{
	    ...
	    for(i = 0; i < command->sessionNum; i++)
		{
		    ...
		    if(!IS_ATTRIBUTE(s_attributes[i], TPMA_SESSION, continueSession)){
                printf("Not attribute\n");
			SessionFlush(s_sessionHandles[i]);
            }else{
                printf("Continue Session\n");
            }
		}
	}
    return;
