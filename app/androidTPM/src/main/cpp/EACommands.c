...
TPM2_PolicySigned(
    ...
    if (session->attributes.isTrialPolicy == CLEAR) {
        ...
        if (authHash.t.size == 0) {
            printf("Authhash is 0\n");
            return TPM_RCS_SCHEME + RC_PolicySigned_auth;
        }
        //  nonceTPM
        printf("Updating with nonce (%d): ", in->nonceTPM.t.size);
        for (int i = 0; i < in->nonceTPM.t.size; i++)
            printf("%02x ", in->nonceTPM.t.buffer[i]);
        printf("\n");
        CryptDigestUpdate2B(&hashState, &in->nonceTPM.b);
        //  expiration
        CryptDigestUpdateInt(&hashState, sizeof(UINT32), in->expiration);
        printf("Updating with expiration: %d\n", in->expiration);
        //  cpHashA
        printf("Updating with cpHash (%d): ", in->cpHashA.t.size);
        for (int i = 0; i < in->cpHashA.t.size; i++)
            printf("%02x ", in->cpHashA.t.buffer[i]);
        printf("\n");
        CryptDigestUpdate2B(&hashState, &in->cpHashA.b);
        //  policyRef
        printf("Updating with policyRef (%d): ", in->policyRef.t.size);
        for (int i = 0; i < in->policyRef.t.size; i++)
            printf("%02x ", in->policyRef.t.buffer[i]);
        printf("\n");
        ...
        printf("Authhash: ");
        for (int i = 0; i < authHash.t.size; i++)
            printf("%02x ", authHash.t.buffer[i]);
        printf("\n");
    ...
    printf("Session after PolicySigned: ");
    for(int i = 0; i < session->u2.policyDigest.t.size; i++){
        printf("%02x ", session->u2.policyDigest.t.buffer[i]);
    }
    printf("\n");
    // Command Output
...
TPM2_PolicyPCR(
    ...
    if (session->attributes.isTrialPolicy == CLEAR) {
        ...
        if (in->pcrDigest.t.size != 0) {
            ...
            if (!MemoryEqual2B(&in->pcrDigest.b, &pcrDigest.b)) {
                printf("Error in expected state\n");
                return TPM_RCS_VALUE + RC_PolicyPCR_pcrDigest;
            } else printf("PCR OK\n");
        }
        else printf("PCRDigest 0\n");
    } else {
        printf("Trial?\n");
    ...
    CryptHashEnd2B(&hashState, &session->u2.policyDigest.b);
    printf("Session after PolicyPCR: ");
    for(int i = 0; i < session->u2.policyDigest.t.size; i++){
        printf("%02x ", session->u2.policyDigest.t.buffer[i]);
    }
    printf("\n");
...
TPM2_PolicyCommandCode(
    ...
    printf("Executing policycommandcode\n");
    SESSION *session;
...
TPM2_PolicyAuthorize(
    ...
    if (digestSize != (in->keySign.t.size - 2)) {
        printf("Digest size not matching keysize\n");
    ...
    if (session->attributes.isTrialPolicy == CLEAR) {
        ...
        if (!MemoryEqual2B(&session->u2.policyDigest.b,
                           &in->approvedPolicy.b)) {
            printf("Not matching he approved policy!\n");
            printf("Session be:\t");
            for (int i = 0; i < SHA256_DIGEST_SIZE; i++)
                printf("%02x ", session->u2.policyDigest.b.buffer[i]);
            printf("\nBut Authorized:\t");
            for (int i = 0; i < SHA256_DIGEST_SIZE; i++)
                printf("%02x ", in->approvedPolicy.b.buffer[i]);
            printf("\n");
